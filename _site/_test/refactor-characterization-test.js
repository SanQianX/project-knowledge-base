...
assert.strictEqual((ui.match(/class="shell"/g) || []).length, 1, 'production must contain one full product shell');
assert.strictEqual((ui.match(/id="settings-logs"/g) || []).length, 1, 'production must contain one Settings logs view');
