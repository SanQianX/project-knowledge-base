...
function buildHookBody({ projectId, triggerScriptPath, nodeExecutable = process.execPath }) {
  ...
  // T02 runtime contract: Git invokes the hook from a normal shell, which does
  // NOT inherit ELECTRON_RUN_AS_NODE=1 the way the Desktop backend does.
  // Without this prefix, the Electron executable (when used as nodeExecutable
  // in a packaged Desktop install) would launch as a GUI product instead of
  // executing the trigger as Node. Pure Node ignores the env var, so the
  // CLI install path is unaffected.
  return `#!/bin/sh
${markerFor(projectId)}
# Generated by project-knowledge. It captures a durable boundary before best-effort notification.
REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
ELECTRON_RUN_AS_NODE=1 ${shellQuote(toGitBashPath(nodeExecutable))} ${shellQuote(toGitBashPath(trigger))} \\
