...
function promptsPath() {
  ...
  return path.join(getDataDir(), cfg.promptsFileName || 'claude-prompts.json');
}

function bundledPromptsPath() {
  const cfg = getWorkbenchConfig();
  return cfg.bundledPromptsPath || path.resolve(__dirname, '..', '..', 'assets', 'claude-prompts.json');
}

function readBundledPrompts() {
  const file = bundledPromptsPath();
  if (!fs.existsSync(file)) return { schema: SCHEMA, prompts: {} };
  try {
    return normalizePromptsConfig(JSON.parse(fs.readFileSync(file, 'utf-8')));
  } catch {
...
function readPrompts() {
  ...
  try {
    const result = withBundledDefaults(JSON.parse(fs.readFileSync(file, 'utf-8')));
...
Object.defineProperties(exported, {
  PROMPTS_PATH: { enumerable: true, get: promptsPath },
  BUNDLED_PROMPTS_PATH: { enumerable: true, get: bundledPromptsPath },
});
