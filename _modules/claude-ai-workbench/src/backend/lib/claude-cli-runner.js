...
function sessionDirName() {
  return getWorkbenchConfig().sessionDirName || 'claude-workbench';
}

function sessionSchema() {
  return getWorkbenchConfig().sessionSchema || 'claude-workbench-session/v1';
}

function projectsPath() {
  const cfg = getWorkbenchConfig();
  return path.join(kbRoot(), cfg.projectsFileName || 'projects.json');
}
...
function applyConfiguredProcessEnv(session) {
  const cfg = getWorkbenchConfig();
  if (!cfg.suppressHooksEnv) return;
  session.claudeEnv = {
    ...(session.claudeEnv || {}),
    [cfg.suppressHooksEnv]: cfg.suppressHooksValue || '1',
  };
}

function fakeClaudeEnabled() {
  const cfg = getWorkbenchConfig();
  const primary = cfg.fakeClaudeEnvVar || 'KB_AUTOMATION_FAKE_CLAUDE';
  return process.env[primary] === '1' || process.env.KB_AUTOMATION_FAKE_CLAUDE === '1';
}
...
function createSession({ projectSlug, projectPath, kbPath, promptKey, source = 'manual', metadata = null }) {
  ...
  const session = {
    ...
    projectSlug,
    projectPath,
    kbPath: kbPath || path.join(projectsRoot(), projectSlug),
    promptKey,
    state: 'idle',
    model: null,
...
function sessionRecordPath(session) {
  ...
  return path.join(aiWorkspace.ensureProjectAIPath(session.projectSlug), sessionDirName(), `${session.sessionId}.json`);
}
...
function readPersistedRecord(file) {
  try {
    ...
    if (parsed && sessionSchemas().includes(parsed.schema) && parsed.sessionId) return parsed;
  } catch {}
  return null;
}
...
function scanPersistedRecords(projectSlug = null) {
  ...
  try {
    ...
    projectDirs = Object.entries(projects || {})
      ...
      .map(([slug, cfg]) => ({ name: slug, kbPath: cfg.kbPath || path.join(projectsRoot(), slug) }));
  } catch {
    try {
      ...
      projectDirs = fs.readdirSync(root, { withFileTypes: true })
        ...
        .map(entry => ({ name: entry.name, kbPath: path.join(root, entry.name) }));
    } catch { return []; }
  }
  const records = [];
  for (const entry of projectDirs) {
    const dirs = sessionDirNames().flatMap(dirName => [
      path.join(aiWorkspace.projectAIPath(entry.name), dirName),
      path.join(entry.kbPath, '_ai', dirName),
    ]);
...
function restoreSessionFromDisk(sessionId) {
  ...
  const kbPath = liveKbPath || record.kbPath || path.join(projectsRoot(), record.projectSlug);
  ...
  const session = {
    ...
    selectedModel: record.selectedModel || record.model || null,
    ...
    commands: inheritConversation && Array.isArray(record.commands) ? record.commands : [],
    commandDiscovery: null,
    safetyPolicy: null,
  };
  sessions.set(sessionId, session);
...
function buildClaudeEnvFromProfile(profile) {
  ...
  const mainModel = profile.mainModel || profile.models && profile.models.default || profile.model || '';
  ...
  if (apiKey) {
    // Coding-plan keys (sk-cp-*) authenticate through ANTHROPIC_AUTH_TOKEN's
    // Bearer header only; adding ANTHROPIC_API_KEY makes Claude Code send the
    // x-api-key header too, which MiniMax-style gateways reject outright.
    env.ANTHROPIC_AUTH_TOKEN = String(apiKey);
  }
...
function profileMainModel(profile) {
  ...
  return String(profile.mainModel || profile.models && profile.models.default || profile.model || '').trim();
}
...
function applyAiProfileToSession(session, aiProfile) {
  ...
  session.aiProfileId = aiProfile.id || null;
  ...
  session.contextWindow = resolveContextWindow({
    // The profile default (200000) must not shadow model auto-detection
    // (e.g. MiniMax 1M); only an explicit non-default override wins.
    profileContextWindow: aiProfile && Number(aiProfile.contextWindow) !== 200000 ? aiProfile.contextWindow : undefined,
    model: session.model || profileMainModel(aiProfile),
  });
  // Model/provider changes keep claudeSessionId: env and model are per-query
  // SDK options, so the conversation resumes under the new selection the
  // same way the CLIs treat the model as a per-request parameter.
  return { resetConversation: false, model: session.model || null };
}

// Rebind a session to another AI profile/model (or back to the agent's own
// login with a null profile). The conversation continues: claudeSessionId is
// preserved and the next SDK query resumes it under the new env/model.
function updateSelection(sessionId, aiProfile, selectedModel) {
  const session = getSession(sessionId);
  if (!session) throw Object.assign(new Error('session not found'), { status: 404 });
  if (session.subprocess) {
    throw Object.assign(new Error(`a subprocess is already running on session ${sessionId}`), { status: 409 });
  }
  const fromAiProfileId = session.aiProfileId || null;
  const fromModel = session.model || null;
  const profile = aiProfile && typeof aiProfile === 'object' ? aiProfile : null;
  session.aiProfileId = profile && profile.id || null;
  session.selectedModel = selectedModel || (profile && profileMainModel(profile)) || null;
  // An agent-owned turn must not inherit the previous provider's model id;
  // null lets the agent default apply until the next init re-syncs.
  session.model = session.selectedModel || null;
  session.claudeEnv = buildClaudeEnvFromProfile(profile);
  session.runner = 'sdk';
  session.contextWindow = resolveContextWindow({
    profileContextWindow: profile && Number(profile.contextWindow) !== 200000 ? profile.contextWindow : undefined,
    model: session.model || profileMainModel(profile),
  });
  applyConfiguredProcessEnv(session);
  persistSession(session);
  emit(session, {
...
function emitImageContent(session, block, name = 'image') {
  ...
  if (source.type === 'base64' && source.data) {
    emit(session, { type: 'claude/image', name, mediaType, data: `data:${mediaType};base64,${source.data}` });
  } else if (source.type === 'url' && source.url) {
    emit(session, { type: 'claude/image', name, mediaType, url: source.url });
  }
}

function emitNestedImages(session, content, name) {
  const blocks = Array.isArray(content) ? content : [];
  for (const block of blocks) {
    if (!block) continue;
    if (block.type === 'image') emitImageContent(session, block, name);
    else if (block.type === 'tool_result' && Array.isArray(block.content)) emitNestedImages(session, block.content, name || 'tool result');
  }
}
...
function handleNdjsonLine(session, line, toolInputBuffers) {
  ...
  if (t === 'system' && parsed.subtype === 'local_command_output') {
    emit(session, { type: 'claude/system-prompt', text: String(parsed.content || '') });
    return;
  }
  ...
  if (t === 'user' || t === 'assistant') {
    ...
    if (t === 'assistant' && parsed.message && Array.isArray(parsed.message.content)) {
      ...
      emitNestedImages(session, parsed.message.content, 'Claude image');
    }
    if (t === 'user' && parsed.message && Array.isArray(parsed.message.content)) {
      for (const block of parsed.message.content) {
        if (block && block.type === 'tool_result') {
          // The SDK reports tool outcomes as user-message tool_result blocks;
          // surface them as first-class tool-result events like the CLI drivers.
          emit(session, {
            type: 'claude/tool-result',
            id: block.tool_use_id,
            output: toolResultText(block.content).slice(0, 4000),
            isError: block.is_error === true,
          });
          emitNestedImages(session, block.content, block.name || 'Tool image');
        }
      }
    }
    return;
...
const EFFORT_THINKING_BUDGETS = { low: 4096, medium: 8192, high: 16384, max: 31999 };

function normalizeEffortLevel(value) {
  const level = String(value || '').trim().toLowerCase();
  return Object.prototype.hasOwnProperty.call(EFFORT_THINKING_BUDGETS, level) ? level : null;
}

function startChatSession({ slug, projectPath, kbPath, aiProfile, permissionMode = 'default', effort }) {
  ...
  session.effort = normalizeEffortLevel(effort);
  emit(session, {
...
async function sendInput(sessionId, text, aiProfile = null, opts = {}) {
  ...
  emit(session, { type: 'claude/user-prompt', text, attachments: Array.isArray(opts.displayAttachments) ? opts.displayAttachments : [], isFollowUp: !isFresh, isFresh });
  const sdkOverrides = buildSdkOverridesFromProfile(aiProfile);
  ...
  const turnOpts = {
    ...
    effort: opts.effort || undefined, // per-message override beats the session-level default
  };
...
function createSdkPrompt(text, attachments) {
  const items = Array.isArray(attachments) ? attachments : [];
  const images = items.filter(item => item && item.data && /^image\//.test(item.mediaType));
  const textFiles = items.filter(item => item && item.data && !/^image\//.test(item.mediaType));
  const prompt = [String(text || '').trim(), ...textFiles.map(item => {
    const name = String(item.name || 'attachment.txt').replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const content = Buffer.from(item.data, 'base64').toString('utf8').replace(/<\/attachment>/gi, '&lt;/attachment&gt;');
...
async function listSupportedCommands(sessionId, aiProfile = null) {
  const session = getSession(sessionId);
  if (!session) throw new Error(`session not found: ${sessionId}`);
  if (Array.isArray(session.commands) && session.commands.length) return session.commands;
  if (session.commandDiscovery) return session.commandDiscovery;
  session.commandDiscovery = (async () => {
    if (aiProfile) {
      applyAiProfileToSession(session, aiProfile);
      applyConfiguredProcessEnv(session);
    }
    ...
    const envModel = session.claudeEnv && session.claudeEnv.ANTHROPIC_MODEL ? String(session.claudeEnv.ANTHROPIC_MODEL) : null;
    const overrides = buildSdkOverridesFromProfile(aiProfile);
    const sdkOptions = {
      cwd: session.kbPath,
      env: { ...process.env, ...(session.claudeEnv || {}), FORCE_COLOR: '0', NO_COLOR: '1' },
      model: envModel || session.model || null,
      tools: { type: 'preset', preset: 'claude_code' },
      settingSources: ['project', 'user', 'local'],
      systemPrompt: overrides.systemPrompt || { type: 'preset', preset: 'claude_code' },
      permissionMode: normalizePermissionMode(session.permissionMode || 'default'),
    };
    if (claudeBin.cmd) sdkOptions.pathToClaudeCodeExecutable = claudeBin.cmd;
    const idle = (async function* () { await new Promise(() => {}); })();
    const queryInstance = query({ prompt: idle, options: sdkOptions });
    let timer;
    try {
      const commands = await Promise.race([
        queryInstance.supportedCommands(),
        new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('Claude Code command discovery timed out')), 15000); }),
      ]);
      session.commands = (Array.isArray(commands) ? commands : []).map(item => ({
        name: String(item.name || '').replace(/^\//, ''), description: String(item.description || ''),
        argumentHint: String(item.argumentHint || ''),
        aliases: Array.isArray(item.aliases) ? item.aliases.map(value => String(value).replace(/^\//, '')) : [], source: 'claude-code',
      })).filter(item => item.name);
      emit(session, { type: 'claude/commands', commands: session.commands });
      return session.commands;
    } finally {
      if (timer) clearTimeout(timer);
      try { queryInstance.close(); } catch {}
      session.commandDiscovery = null;
    }
  })();
  return session.commandDiscovery;
}
...
async function runSdkTurn(session, opts, isResume) {
  ...
  const effortLevel = normalizeEffortLevel(opts.effort || session.effort);
  ...
  const sdkOptions = {
    ...
    ...(effortLevel ? { maxThinkingTokens: EFFORT_THINKING_BUDGETS[effortLevel] } : {}),
  ...
  try {
    ...
    for (let attempt = 0; attempt <= retries; attempt += 1) {
      ...
      try {
        queryInstance = query({
          prompt: createSdkPrompt(opts.userPrompt, opts.attachments),
          options: sdkOptions,
        });
        if ((!session.commands || !session.commands.length) && typeof queryInstance.supportedCommands === 'function') {
          queryInstance.supportedCommands().then(commands => {
            session.commands = (Array.isArray(commands) ? commands : []).map(item => ({
              name: String(item.name || '').replace(/^\//, ''), description: String(item.description || ''),
              argumentHint: String(item.argumentHint || ''),
              aliases: Array.isArray(item.aliases) ? item.aliases.map(value => String(value).replace(/^\//, '')) : [], source: 'claude-code',
...
function readStaleThresholdMs() {
  const cfg = getWorkbenchConfig();
  const envName = cfg.staleActiveEnvVar || 'KB_STALE_ACTIVE_MS';
  const raw = process.env[envName] || process.env.KB_STALE_ACTIVE_MS;
