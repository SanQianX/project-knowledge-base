...
function sanitizeTitle(title) {
  const value = String(title === undefined || title === null ? '' : title)
    .replace(/["\\`$\r\n\t\u0000]/g, '')
    .trim()
    .slice(0, 128);
...
function windowsWorkerScript(title) {
...
// screen instead.
function anchorPoint(windowRect) {
...
function syncWorkerRefs(worker) {
...
function ensureWorker(worker, doSpawn, title, startupTimeoutMs) {
...
function startWorker(worker, doSpawn, startupTimeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    if (child.stderr && child.stderr.on) child.stderr.on('data', () => { /* startup chatter */ });
...
function drainWorkerLines(worker) {
...
function handleWorkerLine(worker, line) {
  if (line === '!READY') {
    if (worker.resolveReady) worker.resolveReady();
  } else if (line === '!PONG') {
...
function settleWorkerPick(worker, finish) {
...
async function pickViaWindowsWorker(options) {
  const doSpawn = options.spawn || spawn;
  const timeoutMs = Number.isFinite(options.timeoutMs) && options.timeoutMs > 0
    ? options.timeoutMs
    : DEFAULT_TIMEOUT_MS;
  const startupTimeoutMs = Number.isFinite(options.startupTimeoutMs) && options.startupTimeoutMs > 0
...
async function pickFolderUnlocked(options = {}) {
  ...
  const timeoutMs = Number.isFinite(options.timeoutMs) && options.timeoutMs > 0
    ? options.timeoutMs
    : DEFAULT_TIMEOUT_MS;
  const run = (command, args) => runOneShot(command, args, doSpawn, timeoutMs);
  try {
 ...
 */
async function warmFolderPicker(options = {}) {
 ...
 */
async function pingFolderPicker(options = {}) {
  ...
  await new Promise((resolve, reject) => {
    ...
    if (ping.timer.unref) ping.timer.unref();
    worker.ping = ping;
    syncWorkerRefs(worker);
    if (!worker.child || !worker.child.stdin) {
      worker.ping = null;
...
function runOneShot(command, args, doSpawn, timeoutMs) {
  return new Promise((resolve, reject) => {
    let child;
    try {
...
module.exports = { pickFolder, warmFolderPicker, pingFolderPicker, parseSelectedPath };
